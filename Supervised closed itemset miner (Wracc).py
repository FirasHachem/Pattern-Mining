...
#compute the absolute Wracc score
def Wracc(P, N, p, n):
    coeff = (P / (P + N)) * (N / (P + N))
...
#         check threshold on positive support
        if candi[1] >= mini:
...
#     set threshold to 1
    ...
    # create the big queue [itemset, pos_support, neg_support, wracc]
    for q in queue:
        pos = 0
        neg = 0
        if q in vertic_pos:
    ...
    while QUEUE:
        new_candi = []
        q = QUEUE.pop(0)
#         threshold is the diff of the positive supp and negative
        thresh = abs(q[1] - q[2])
...
#         check threshold
        if  q[1] >= mini:
            ...
            for c in reversed(new_candi):
                ...
                # sort the queue to speed up the search on the absolute wrocc score
            QUEUE = sorted(QUEUE, key=lambda x: x[3], reverse=True)
